    ...
    <section className="w-full   mt-16">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end max-[1300px]:flex-col">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Середня вартість замовлення <span className="text-green-500 text-heading4-medium">{totalAverage.toFixed(0)}{Store.currency_sign}</span></h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Середня вр-тсть замовлення: <span className={`${payload[0].value > 0 && "text-green-500"}`}>{Store.currency_sign}{payload[0].value.toFixed(0)}</span></p>
      </div>
    );
  }
  return null;
